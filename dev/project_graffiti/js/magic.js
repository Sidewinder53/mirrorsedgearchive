...
$("#emblemPicker, #framePicker, #backgroundPicker").change(function() {
	...
	$selected_emblem_path = $('#emblemPicker').find(":selected").attr("data-img-src");
	$selected_frame_path = $('#framePicker').find(":selected").attr("data-img-src");
	$selected_background_path = $('#backgroundPicker').find(":selected").attr("data-img-src");

	$selected_emblem_alt = $("#emblemPicker option:selected").attr(("data-img-alt"));
	$selected_frame_alt = $("#framePicker option:selected" ).attr(("data-img-alt"));
	$selected_background_alt = $("#backgroundPicker option:selected" ).attr(("data-img-alt"));
	...
	if ($selected_emblem == "random_emblem")
		$selected_emblem_path = "";

	if ($selected_frame == "random_frame")
		$selected_frame_path = "";
	...
	if ($selected_emblem == "no_emblem")
		$selected_emblem_path = "resources/img/empty.png";

	if ($selected_frame == "no_frame")
		$selected_frame_path = "resources/img/empty.png";

	if ($selected_background == "no_background")
		$selected_background_path = "resources/img/empty.png";

	if ($selected_emblem == "random_emblem" || $selected_frame == "random_frame" || $selected_background == "random_background" ) {
...
function updateBackgroundGradient(){
	if ($selected_background == "random_background" || $selected_background == "no_background") {
		$("#background_preview_background").css({
			'background': '-moz-linear-gradient(-45deg, rgba(0,0,0,0.95) 0%, rgba(0,0,0,0.5) 100%)',
			'background': '-webkit-linear-gradient(-45deg, rgba(0,0,0,0.95) 0%,rgba(0,0,0,0.5) 100%)',
			'background': 'linear-gradient(135deg, rgba(0,0,0,0.95) 0%,rgba(0,0,0,0.5) 100%)',
			'filter': 'progid:DXImageTransform.Microsoft.gradient( startColorstr="#f2000000", endColorstr="#80000000",GradientType=1 )'
		})
	} else {
		$("#background_preview_background").css({
			'background' : '-moz-linear-gradient(left, rgba(0,0,0,0.6) 0%, rgba(0,0,0,1) 50%, rgba(0,0,0,0.6) 100%)',
			'background' : '-webkit-linear-gradient(left, rgba(0,0,0,0.6) 0%,rgba(0,0,0,1) 50%,rgba(0,0,0,0.6) 100%)',
...
function adjustBackgroundGradient(){
	if ($("#resultpng").height == $("#print").height) {
		...
		$("#print").css({
			'background': '-moz-linear-gradient(top, rgba(0,0,0,0.5) 0%, rgba(0,0,0,1) 50%, rgba(0,0,0,0.5) 100%)',
			'background': '-webkit-linear-gradient(top, rgba(0,0,0,0.5) 0%,rgba(0,0,0,1) 50%,rgba(0,0,0,0.5) 100%)',
			'background': 'linear-gradient(to bottom, rgba(0,0,0,0.5) 0%,rgba(0,0,0,1) 50%,rgba(0,0,0,0.5) 100%)',
...
$("#debugButton").click(function() {
	...
	$('#resultpng').css({'max-width' : $("#print").width(), 'max-height' : $("#print").height() });
...
$("#downloadButton").click(function() {
	$('#downloadButtonHelper').get(0).click();
});
